...
def create_handle_browse(button_type, label):
    def handle_browse(event=None):
        if button_type == "input":
            ...
            config_file_path = filedialog.askopenfile("r")
...
def init_gui():
    ...
    checkbox_recursive = tk.Checkbutton(
        master=_main_window, variable=recursive_var, text="Recursive"
    )
    ...
    button_run = tk.Button(
        ...
        command=create_run_command(
            _main_window, entry_start_count, recursive_var
        ),  # noqa: E501
    )
